...
<html lang="en">
  ...
  <body class="">
    ...
    <section class="text-center">
      ...
      <div class="card mx-4 mx-md-auto shadow-5-strong bg-body-tertiary" style="
        margin-top: -100px;
        ...
        <div class="card-body py-3 px-3 md-3   bg-warning ">
          ...
          <div class="row d-flex justify-content-center">
            <div class="col-lg-10"> <!-- Adjusting width to 10 columns -->
              <h2 class="fw-bold mb-5   mt-4">Sign up now</h2>
             <?php 
    ...
    if ($rows == 1) { // User found
        $row = $check_result->fetch_assoc();
        if ($row['role'] == 'admin') { 
            session_start(); // Start session if not already started
            $_SESSION['id'] = $row['id']; // Store user ID in session
            $_SESSION['role'] = $row['role']; // Store user role in session
            header('location: admin_panel.php'); // Redirect to admin panel page
            exit();
        } else { // Agar user ka role admin nahi hai
          
       
            echo '<script>alert("Access denied!Only admin can access this page");</script>';
        }
    } else { 
      echo '<script>alert("Incorrect useremail or password. Please try again.");</script>';
    }
}
...
<form method="post">
                ...
                <!-- Submit button -->
                <div class="pt-1 mb-4">
                  <button class="btn btn-dark btn-lg btn-block" name="login-btn" type="submit">Login</button>
                </div>
              </form>

             
              <p  class="fw-semibold  fs-5">Don't have an account? <a href="register.php" class="link-dark"   class="fw-semibold fs-5">Register here</a></p>
